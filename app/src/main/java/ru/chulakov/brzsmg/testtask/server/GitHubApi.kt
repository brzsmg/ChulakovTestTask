...
import ru.chulakov.brzsmg.testtask.Model.Repository
...
interface GitHubApi {
    ...
    @GET("/users")
    fun requestUsers(@Query("page") page : Int, @Query("per_page") per_page : Int) : Single<Response<List<User>>>

    @GET("/users/{login}")
    fun requestUser(@Path("login") login : String) : Single<Response<User>>

    @GET("/search/users")
    fun requestSearchUsers(@Query("page") page : Int, @Query("per_page") per_page : Int, @Query("q") q : String) : Single<Response<SearchResults<User>>>
